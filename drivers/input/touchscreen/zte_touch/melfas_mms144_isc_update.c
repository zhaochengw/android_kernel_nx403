...
static void hw_reboot_normal(struct zte_ctp_dev *ts)
{
	unsigned int reset_gpio = ts->pdata->zte_tp_interface.cs;
...
int melfas_mms144_isc_fw_download(struct zte_ctp_dev *ts, const u8 *data, size_t len)
{
    u8 *buff;
    u16 crc_buf;
    int src_idx;
    ...
    if (!buff)
    {
        TP_ERR("%s: failed to allocate memory\n", __func__);
        kfree(buff);
    ...
    *buff = ISC_ENTER_ISC_DATA;
    ret = i2c_smbus_write_byte_data(ts->client, ISC_ENTER_ISC_CMD, *buff);
    if (ret < 0)
    {
        TP_ERR("fail to enter ISC mode(err=%d)\n", ret);
        goto fail_to_isc_enter;
    }
    usleep_range(10000, 20000);
    dev_info(&ts->client->dev, "Enter ISC mode\n");

    /*enter ISC update mode */
    *buff = ISC_ENTER_UPDATE_DATA;
    ret = i2c_smbus_write_i2c_block_data(ts->client, ISC_CMD, ISC_ENTER_UPDATE_DATA_LEN, buff);
    if (ret < 0)
    {
        TP_ERR("fail to enter ISC update mode(err=%d)\n", ret);
        goto fail_to_isc_update;
    }
    dev_info(&ts->client->dev, "Enter ISC update mode\n");
    ...
    *(buff + 1) = ISC_DATA_WRITE_SUB_CMD;
    for (i = 0; i < ISC_PKT_NUM; i++)
    {
        ...
        *(buff + ISC_PKT_DATA_SIZE + ISC_PKT_HEADER_SIZE) = crc_buf >> 8 & 0xFF;
        ret = i2c_master_send(ts->client, buff, ISC_PKT_SIZE);
        if (ret < 0)
        {
            TP_ERR("fail to firmware writing on packet %d.(%d)\n", i, ret);
            goto fail_to_fw_write;
        }
        usleep_range(1, 5);

        /* confirm CRC */
        ret = i2c_smbus_read_byte_data(ts->client, ISC_CHECK_STATUS_CMD);
        if (ret == ISC_CONFIRM_CRC)
        {
            dev_info(&ts->client->dev, "updating %dth firmware data packet.\n", i);
        }
        else
        {
            TP_ERR("fail to firmware update on %dth (%X).\n", i, ret);
            ret = -1;
...
fail_to_fw_write:
    ...
    *(buff + 1) = ISC_EXIT_ISC_SUB_CMD2;
    i2c_smbus_write_i2c_block_data(ts->client, ISC_CMD, 2, buff);
    usleep_range(10000, 20000);
