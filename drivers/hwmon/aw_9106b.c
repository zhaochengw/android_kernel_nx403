...
{
	 gpio_set_value(GPIO_29, power_set);
}
			...
			const struct i2c_device_id *dev_id)
{
	...
	ret = gpio_request(GPIO_29, "aw9106b_shdn");
	if (ret) {
		pr_err("%s: fail gpio_request(%d)=%d\n", __func__,GPIO_29, ret);
		goto init_fail;
	}
	gpio_tlmm_config(GPIO_CFG(GPIO_29, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
