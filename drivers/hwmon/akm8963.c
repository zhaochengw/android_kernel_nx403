...
static int create_sysfs_interfaces_debug(struct device *dev)
{
	int i;
...
// modify for prevent input filter data
bool flag_input_filter = false;
	...
	struct i2c_client *client)
{
    ...
    for (i=0; i<10; i++)
    {
	    ...
	    if (err < 0) 
        {
            printk(KERN_INFO "----------%s: %d: retry time %d\n",__func__,__LINE__,i+1);
		    //dev_err(&client->dev,"%s: Can not read WIA.", __func__);
	    }
        else
        {
            break;
    ...
    if (i >= 10)
    {
         printk(KERN_INFO "----------%s: %d: Can not read WIA %d\n",__func__,__LINE__,i+1);   
         return err;
    }
	...
	int *rbuf)
{
    ...
    // ZTEMT ADD by zhubing 
    // modify for prevent input filter data
    if (flag_input_filter)
    {
        rbuf[8] += 10;
        flag_input_filter = false;
    }
    else
    {
        flag_input_filter = true;
    }
    // ZTEMT ADD by zhubing END
	...
	if (ready & MAG_DATA_READY) {
        ...
        if (flag_debug)
        {
            SENSOR_LOG_ERROR("mag data: x = %d, y = %d, z = %d\n",rbuf[5],rbuf[6],rbuf[7]);
        }
	}
...
static irqreturn_t akm8963_irq(int irq, void *handle)
{
    ...
    for (i=0; i<100; i++)
    {
        if (akm8963_i2c_rxdata(akm->i2c, buffer, 1) < 0)
        {
        ...
        {
            buffer[0] = AK8963_REG_ST1;
            if (akm8963_i2c_rxdata(akm->i2c, buffer, SENSOR_DATA_SIZE) < 0)
            {
                dev_err(&akm->i2c->dev, "%s failed.", __func__);
                goto work_func_end;
            }
    ...
    if (i>=100)
    {
       SENSOR_LOG_ERROR("get akm8963 data error");
       return IRQ_HANDLED;
    }
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef FEATURE_ZTEMT_SENSORS_COMPASS_UES_INT_MODE
    if (pdata->gpio_init())
	   	SENSOR_LOG_ERROR("%s: sensor_akm8963_gpio_init faild\n", __func__);
	else
		SENSOR_LOG_ERROR("%s: sensor_akm8963_gpio_init succesfull\n", __func__);
    AKM8963_INT = MSM_GPIO_TO_INT(pdata->gpio_DRDY);
	s_akm->irq = AKM8963_INT ;//
#else
	s_akm->irq = client->irq;
#endif

	if (s_akm->irq == 0) {
        SENSOR_LOG_ERROR("----------%s: %d: IRQ is not set.\n", __func__,__LINE__);
    ...
    err = create_sysfs_interfaces_debug(&client->dev);
    if (err < 0) {
		dev_err(&client->dev,
			"%s: create sysfs failed.", __func__);
    }
...
#ifdef FEATURE_ZTEMT_SENSORS_COMPASS_UES_INT_MODE
	free_irq(AKM8963_INT, client);
       if (pdata->gpio_exit())
	   	SENSOR_LOG_ERROR("%s: sensor_akm8963_gpio_exit faild\n", __func__);
	else
		SENSOR_LOG_ERROR("%s: sensor_akm8963_gpio_exit succesfull\n", __func__);
#endif	
...
{
	SENSOR_LOG_ERROR( "AKM8963 compass driver: initialize.");
	return i2c_add_driver(&akm8963_driver);
}

static void __exit akm8963_exit(void)
{
	SENSOR_LOG_ERROR( "AKM8963 compass driver: release.");
