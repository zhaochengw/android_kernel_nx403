...
static long taos_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        {
            ret = schedule_delayed_work(&taos_datap->als_poll_work,0);
            break;
        }
...
static int taos_prox_on(void)
{
    ...
    if (j==0)
    	{
    taos_cfgp->prox_threshold_hi = ((((prox_max - prox_mean) * prox_calibrate_hi_param) + 50)/100) + prox_mean+45;
    taos_cfgp->prox_threshold_lo = ((((prox_max - prox_mean) * prox_calibrate_lo_param) + 50)/100) + prox_mean+15;
...
static int taos_prox_calibrate(void)
{
    ...
    if ((ret = (i2c_smbus_write_byte_data(taos_datap->client, (TAOS_TRITON_CMD_REG | TAOS_TRITON_CNTRL), reg_cntrl))) < 0) {
		...
		if((prox_mean<33) ||(prox_min==0))
				{

				prox_config_offset_param=0xB1;
				taos_cfgp->prox_config_offset = prox_config_offset_param;
					if ((ret = (i2c_smbus_write_byte_data(taos_datap->client, (TAOS_TRITON_CMD_REG|0x1E), taos_cfgp->prox_config_offset))) < 0) {
					    printk(KERN_ERR "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_on\n");
					    return (ret);
		...
		if((prox_max-prox_mean) < 30 ||(prox_max-prox_min ) < 60)
			{
			taos_cfgp->prox_threshold_hi = ((((prox_max - prox_mean) * prox_calibrate_hi_param) + 50)/100) + prox_mean +33  ;
			taos_cfgp->prox_threshold_lo = ((((prox_max - prox_mean) * prox_calibrate_lo_param) + 50)/100) + prox_mean+22;
			}
		else
			{
			taos_cfgp->prox_threshold_hi = ((((prox_max - prox_mean) * prox_calibrate_hi_param) + 50)/100) + prox_mean;
