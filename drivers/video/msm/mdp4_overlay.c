				  ...
				  struct mdp4_overlay_pipe *pipe)
{
	...
	if ((pipe->src_h < pipe->dst_h) && (pipe->src_w < pipe->dst_w)) { 
		//pr_err("sss pipe->src_h=%d, pipe->dst_h=%d\n",pipe->src_h,pipe->dst_h);
		//pr_err("sss pipe->src_w=%d, pipe->dst_w=%d\n",pipe->src_w,pipe->dst_w);
	...
	if ((pipe->src_h > pipe->dst_h) || (pipe->src_w > pipe->dst_w)) { 
		//pr_err("sss pipe->src_h=%d, pipe->dst_h=%d\n",pipe->src_h,pipe->dst_h);
		//pr_err("sss pipe->src_w=%d, pipe->dst_w=%d\n",pipe->src_w,pipe->dst_w);
		if ((pipe->src_h <= (pipe->dst_h * 8)) && (pipe->src_w <= (pipe->dst_w * 8))) { 
			pr_err("**** %s: < 1/8x Overscaling case!\n", __func__); 
			pipe->req_clk = mdp_max_clk; 
			return 0; 
		} 
	}	
