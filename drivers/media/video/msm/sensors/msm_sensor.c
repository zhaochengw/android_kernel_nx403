...
#ifdef CONFIG_ZTEMT_CAMERA_NX403
        gpio_request_one(BACK_CAM_AVDD_EN,GPIOF_OUT_INIT_LOW, "back_cam_avdd_en");	
	if (rc) {
		pr_err("request BACK_CAM_AVDD_EN failed rc=%d\n", rc);
	}		
	
	gpio_direction_output(BACK_CAM_AVDD_EN, 1);
#endif
...
#ifdef CONFIG_ZTEMT_CAMERA_NX403
/////tyj add 
	gpio_direction_output(BACK_CAM_AVDD_EN, 0);
      gpio_free(BACK_CAM_AVDD_EN);	
#endif	
