...
#include "wcd9310.h"
#ifdef CONFIG_ZTEMT_AUDIO_Z5MINI
...
static void tabla_codec_pause_hs_polling(struct snd_soc_codec *codec)
{
	...
	//msleep(250);  
	/*ztemt audio add by sunli for the headset call noise and headset tone delay when quickly swithing*/
	msleep(20);   
...
static void tabla_hs_insert_irq_gpio(struct tabla_priv *priv, bool is_removal)
{
	struct snd_soc_codec *codec = priv->codec;
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(codec);
#ifdef CONFIG_ZTEMT_AUDIO_DEBUG
	printk("[ZTEMT AUDIO]enter %s",__func__);
#endif
